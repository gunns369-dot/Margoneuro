...
function onExpMapChanged(currMap, now) {
    ...
    const entryCd = 1200 + Math.floor(Math.random() * 301);
    ...
    if (isExpMap) {
        ...
        if (!routeState.targetExpMap || normMapName(routeState.targetExpMap) === normMapName(currMap) || shouldClearCurrentMapNow(currMap)) {
            window.rushNextMap = null;
        }
    }
    return true;
...
function runExpLogic() {
    ...
    if (shouldFightHere && target) {
        ...
        if (exactDist <= 1) { // Jesteśmy przy celu
            ...
            const nextTarget = nextChoice?.mob || null;
            const nextPathData = nextTarget ? getPathToAdjacentTile(nextTarget.x, nextTarget.y, distMap) : null;

            if (nextTarget && nextPathData?.stand && now - window.expStandStillStart > 650 && now > nextAllowedClickTime) {
        ...
        if (!sameTarget) {
            ...
            if (heroMoved) {
                ...
                approachState.anchorAt = now;
                approachState.failCount = 0;
            } else if (now - (approachState.anchorAt || now) > 3200) {
            ...
            if (!isMoving) {
                if (window.lastStuckCheckPos.x === currentX && window.lastStuckCheckPos.y === currentY && window.lastStuckCheckPos.map === currentMap) {
                    ...
                    if (window.stuckIdleCount >= 8) {
                        ...
                        if (typeof Engine?.hero?.clearAutoPathOfHero === 'function') Engine.hero.clearAutoPathOfHero();
                        ...
                        if (!jitter) {
                            const randomNeighbors = [[-1,-1],[-1,1],[1,-1],[1,1]].sort(() => Math.random() - 0.5);
                            jitter = randomNeighbors.map(([dx, dy]) => ({ x: currentX + dx, y: currentY + dy }))
                                .find(p => !(typeof Engine?.map?.checkCollision === 'function' && Engine.map.checkCollision(p.x, p.y)));
                        }
                        if (jitter && typeof Engine?.hero?.autoGoTo === 'function') Engine.hero.autoGoTo({x: jitter.x, y: jitter.y});
                        window.stuckIdleCount = 0;
                    }
                } else {
...
// --- RYSOWANIE LINII DO CELU ---
    ...
    try {
        const infoPanel = document.getElementById('margoRadarInfoPanel');
        if (infoPanel) {
            const decisionText = window.expDecisionInfo ? `<div style="padding:4px 0 6px 0; color:#9adcf7; border-bottom:1px solid rgba(255,255,255,0.09); margin-bottom:4px;">[Radar] ${window.expDecisionInfo}</div>` : '';
